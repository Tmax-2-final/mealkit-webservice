...
@Controller
public class UserBasedRecommendCsvGradeController {
	private static Logger log = LoggerFactory.getLogger(UserBasedRecommendCsvGradeController.class);

	@ResponseBody
	@RequestMapping(value = {"/user/grade"}, method = RequestMethod.GET)
	public String recommend(HttpServletRequest request) {
		
		try {
			URL dataFilePath = getClass().getResource("/dataset_grade.csv"); // 데이터 파일 경로
			...
			List<RecommendedItem> recommendations = recommender.recommend(1000036529, 10); //추천 조회 user_id, 갯수
